# darkdevs - telegram group management bot
crdidts - @dynamicsouls 
...
from group_bot import OWNER_ID, BOT_ID, OPERATORS, dynamicsouls, bot
from group_bot.services.mongo import db
from group_bot.services.redis import redis
from group_bot.services.telethon import tbot
...
async def fed_post_log(fed, text):
    ...
    with suppress( NeedAdministratorRightsInTheChannel, ChatNotFound):
...
def get_current_chat_fed(func):
    async def wrapped_1(*args, **kwargs):
        ...
        if not (fed := await get_fed_f(message)):
            await message.reply(await get_string(real_chat_id, "feds", 'chat not joined fed'))
            return
    ...
    def wrapped(func):
        async def wrapped_1(*args, **kwargs):
                ...
                if len(text_args) >= 1:
                    if text_args[0].count('-') == 4:
                        ...
                        if not (fed := await get_fed_by_id(text_args[0])):
                            await message.reply(strings['Invalif fed id'])
                            return
                    else:
                        text = " ".join(text_args)
            ...
            if not fed:
                if not (fed := await get_fed_f(message)):
                    if not skip_no_fed:
                        await message.reply(strings['chat not joined fed'])
                        return
                    else:
                        fed = None
        ...
        if message.text:
            ...
            if not len(text_args) < 2 and text_args[1].count('-') == 4:
                if not (fed := await get_fed_by_id(text_args[1])):
                    await message.reply(await get_string(real_chat_id, "feds", 'Invalid fed id'))
                    return
...
@dynamicsouls.register(cmds=['newfed', 'fnew'])
@need_args_dec()
...
@dynamicsouls.register(cmds=['joinfed'])
@need_args_dec()
...
async def join_fed(message, chat, strings):
    ...
    if not (fed := await get_fed_by_id(fed_id)):
        await message.reply(strings['Invalid fed id'])
        return

    # Assume chat already joined this/other fed
    if 'chats' in fed and chat_id in fed['chats']:
        await message.reply(strings['This chat has been joined fed already'])
        return
...
@dynamicsouls.register(cmds=['leavefed'])
@chat_connection(admin=True, only_groups=True)
...
@dynamicsouls.register(cmds='subfed')
@need_args_dec()
@get_current_chat_fed
@is_fed_owner
@get_strings_dec("feds")
...
@dynamicsouls.register(cmds='unsubfed')
@need_args_dec()
@get_current_chat_fed
@is_fed_owner
@get_strings_dec("feds")
...
@dynamicsouls.register(cmds='fpromote')
@get_fed_user_text()
@is_fed_owner
@get_strings_dec("feds")
...
@dynamicsouls.register(cmds='fdemote')
@get_fed_user_text()
@is_fed_owner
@get_strings_dec("feds")
...
@dynamicsouls.register(cmds=['setfedlog'], only_groups=True)
@get_fed_dec
...
@dynamicsouls.register(cmds=['unsetfedlog'], only_groups=True)
@get_fed_dec
@is_fed_owner
@get_strings_dec("feds")
...
@dynamicsouls.register(cmds=['fedchats', 'fchats'])
@get_fed_dec
@is_fed_admin
@get_strings_dec("feds")
...
@dynamicsouls.register(cmds=['fedadminslist', 'fedadmins'])
@get_fed_dec
@is_fed_admin
@get_strings_dec("feds")
async def fed_admins_list(message, fed, strings):
    text = strings['fedadmins_header'].format(
        fed_name=html.escape(fed['fed_name'], False))
...
@dynamicsouls.register(cmds=['fedinfo'])
@get_fed_dec
@get_strings_dec("feds")
async def fed_info(message, fed, strings):
    text = strings['fedinfo_text']
    ...
    if 'subscribed' in fed and len(fed['subscribed']) > 0:
        text += strings['fedinfo_subs_title']
...
@dynamicsouls.register(cmds=['fban', 'sfban'])
@get_fed_user_text()
@is_fed_admin
@get_strings_dec("feds")
...
@dynamicsouls.register(cmds=['unfban', 'funban'])
@get_fed_user_text()
@is_fed_admin
@get_strings_dec("feds")
...
@dynamicsouls.register(cmds=['delfed')
@get_fed_dec
@is_fed_owner
@get_strings_dec('feds')
...
@dynamicsouls.register(delfed_cb.filter(), f='cb', allow_kwargs=True)
@get_strings_dec('feds')
...
@dynamicsouls.register(regexp='cancel_(.*)', f='cb')
...
@dynamicsouls.register(cmds='renamefed')
@need_args_dec()
@get_fed_dec
@is_fed_owner
@get_strings_dec('feds')
async def fed_rename(message, fed, strings):
    ...
    if new_name == fed['fed_name']:
        await message.reply(strings['renamefed_same_name'])
        return

    await db.feds.update_one({'_id': fed['_id']},
                             {'$set': {'fed_name': new_name}})
    await get_fed_by_id.reset_cache(fed['fed_id'])
    await message.reply(strings['renamefed_success'].format(
        old_name=html.escape(fed['fed_name'], False), new_name=html.escape(new_name, False))
    )


@dynamicsouls.register(cmds=['fbanlist', 'exportfbans', 'fedexport'])
@get_fed_dec
@is_fed_admin
@get_strings_dec('feds')
...
@dynamicsouls.register(cmds=['importfbans', 'fedimport'])
@get_fed_dec
@is_fed_admin
@get_strings_dec('feds')
...
@dynamicsouls.register(state=ImportFbansFileWait.waiting, content_types=types.ContentTypes.DOCUMENT, allow_kwargs=True)
@get_fed_dec
@is_fed_admin
...
@dynamicsouls.register(only_groups=True)
...
@dynamicsouls.register(cmds=['fbanstat'])
...
__help__ = """
There are two ways to use federation 
1. by creating your fed you can join fed in chats
2. you can subsribe your fed 

In both cases blah will help you.
...
<code>(file)</code>: required file, if file isn't provided you will be entered in file state, this means blah  will wait file message from you. Type /cancel to leave from it.
<code>(? )</code>: additional argument

<b>Only Federation owner:</b>
- /newfed (name) or /newfed (name): Creates a new Federation
- /renamefed (?Fed ID) (new name): Renames your federation
- /delfed (?Fed ID) or /delfed (?Fed ID): Removes your Federation
- /fpromote (user) (?Fed ID): Promotes a user to the your Federation
- /fdemote (user) (?Fed ID): Demotes a user from the your Federation
- /subfed (Fed ID): Subscibes your Federation over provided
- /unsubfed (Fed ID): unsubscibes your Federation from provided
- /setfedlog (? Fed ID) (? chat/channel id): Set's a log chat/channel for your Federation
- /unsetfedlog (?Fed ID): Unsets a Federation log chat\channel
- /fedexport (?Fed ID): Exports Federation bans
- /fedimport (?Fed ID) (file): Imports Federation bans

<b>Only  Group owner can :</b>
...
- /sfban (user) (?Fed ID) (?reason): you didn't get notify after fban
- /unfban (user) (?Fed ID) (?reason): Unban, a user from a Federation

<b>Avaible for all users:</b>
- /fstat (?user): Check user's federation ban info
- /fedinfo (?Fed ID): Info about Federation
"""